...
function buildPrompt(size, style, cutout) {
  return (
    '请把「第一张图」（用户照片）转换成 ' +
    size +
    '×' +
    ...
    '\n' +
    (cutout
      ? '「抠图」最重要的要求：人物之外的背景必须 100% 纯白色（RGB 255,255,255）。人物边缘干净利落，人物以外不得出现任何原背景物体、家具、墙面、植物、阴影、渐变或装饰，如同把人物从照片中完整抠出后放在纯白画布上。\n'
...
async function generate(apiKey, model, data) {
  ...
  const payload = {
    model,
    prompt: buildPrompt(size, data.style || '卡通', data.cutout),
    image: images,
    size: GEN_SIZE,
    response_format: 'url',
