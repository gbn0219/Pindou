...
- AI 优化图纸（展示页"AI 优化"按钮）：把**原图（裁剪后 imagePath）和当前图纸一起发送给 AI**，让 AI 基于原图优化图纸（避免自由发挥不像原图）。提示词要求：人物/物体及五官特征与原图一致、边界清晰尽量黑色描边、平滑内部颜色、配色与原图一致、不加文字。后端两种模式（`config.aiOptimize.backend`）：`local` = 本地代理服务 `tools/ai-optimize-server.js`（读取根目录 `.env` 的 `DASHSCOPE_API_KEY`，开发者工具需勾选"不校验合法域名"，仅本机开发用）；`cloud` = 云函数 `ai-optimize-pattern`。结果重新走 `dominantBlocks → mapRgb → mergeGrid → denoiseGrid` 生成新图纸并刷新展示。按次计费，前端有确认弹窗
...
- 云函数 `ai-optimize-pattern`（cloud 模式）需配置环境变量：`DASHSCOPE_API_KEY`（必填）、`DASHSCOPE_MODEL`（可选，默认 `qwen-image-2.0-pro`）；在开发者工具云函数面板或云开发控制台"云函数 → 配置 → 环境变量"中设置
