 ...
 * 交互：屏幕上固定一张 R×C 网格（位置锁死），用户缩放/拖动【图片】对齐网格，
 * 可“自动对齐”（投影法网格线检测预对齐），再微调。方形 MVP（宽==高）。
 */
...
Page({
  ...
  computeGrid() {
    ...
    const cell = Math.max(2, Math.min(innerW / this.data.cols, innerH / this.data.rows))
    this.grid = {
      cell,
      gx: FIT_PAD + (innerW - this.data.cols * cell) / 2,
      gy: FIT_PAD + (innerH - this.data.rows * cell) / 2
    }
  },
  ...
  fitView(iw, ih) {
    const g = this.grid
    const gw = this.data.cols * g.cell
    const gh = this.data.rows * g.cell
  ...
  drawGrid(ctx) {
    ...
    const gw = cols * g.cell
    const gh = rows * g.cell
    // 网格外轻遮罩：强调“网格内才是识别范围”
  ...
  async recognize() {
    ...
    try {
      const ctx = this.fullCanvas.getContext('2d')
      const imageData = ctx.getImageData(0, 0, this.wImg.width, this.wImg.height)
      const { rgbGrid, darkGrid } = scan.scanScreenGrid(imageData, rows, cols, this.grid, this.view)
      ...
      grid = pattern.postProcessGrid(grid, palette, { bgMask })
